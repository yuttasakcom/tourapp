...
module.exports = async (req, res, next) => {
  ...
  const { busPathsSummary, pkgName } = await repo.companyGetBusPathsSummary(
    companyId,
    date,
    pkgId
  )
  ...
  const busPathsContent = map(busPathsSummary, (tourists, busPathName) => {
    index += 1
    const body = []
    const headers = [
      {
    ...
    return [
      {
        text: `แพ็คเกจ ${pkgName} ${busPathName === 'undefined'
          ? 'ตกหล่น (ยังไม่ได้จัดรถ)'
          : busPathName} วันที่ ${dateString}`,
        style: 'header'
      },
      {
        style: 'table',
